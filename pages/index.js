...
export default function PrepTestG1() {
  ...
  // Trial lock (after first 20 questions of module 1)
  ...
  useEffect(() => {
    ...
    async function checkAccessLoop() {
      ...
      try {
        ...
        // No key is OK (free first 20 questions). Just mark checked.
        if (!savedKey) {
          if (!cancelled) {
            setHasAccess(false);
            setAccessChecked(true);
  ...
  const next = () => {
    ...
    // Lock immediately when trying to go past Q20 of first module without access
  ...
  const startByIndex = (startIdx, endIdx, baseNumber) => {
    // If locked, always gate
    if (!ownerOverride && !hasAccess && trialLocked) {
      setTrialLockOn();
      return;
    }

    // If user is not unlocked and tries to start any module other than the first, lock immediately
    ...
    <div style={styles.page}>
      {checkpointOpen && (
        <CheckpointScreen
          ...
          onContinue={() => {
            // If user just finished Q20 of module 1 without access, lock now
            if (!ownerOverride && !hasAccess && isFirstModule && inSet === 20) {
              setCheckpointOpen(false);
              setBlockAnswered(0);
              setBlockCorrect(0);
