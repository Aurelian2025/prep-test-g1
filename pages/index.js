...
const FREE_LOCK_KEY = "g1_free_lock_v1";
const FREE_LIMIT = 20;
...
export default function PrepTestG1() {
  ...
  // Free lock state (after first 20 questions of module 1)
  ...
  useEffect(() => {
    ...
    try {
      if (typeof window !== "undefined") {
        const v = localStorage.getItem(FREE_LOCK_KEY);
        if (v === "1") setFreeLocked(true);
      }
    } catch (_) {}
  ...
  const isFreeUser =
    !hasAccess && !ownerOverride && !freeLocked && globalBase === 0;
  ...
  const submit = () => {
    ...
    // Free access: after answering the 20th question of module 1, lock.
    if (isFreeUser && inSet === FREE_LIMIT) {
      try {
        localStorage.setItem(FREE_LOCK_KEY, "1");
      } catch (_) {}
      setFreeLocked(true);
      setAccessError("");
    }
  ...
  const next = () => {
    ...
    // If free has been locked, prevent moving forward.
    if (!hasAccess && !ownerOverride && freeLocked) return;
  ...
  const startByIndex = (startIdx, endIdx, baseNumber) => {
    ...
    // Free users can only use Start 1–40 and only up to first 20 questions.
  ...
  useEffect(() => {
    ...
    // Free users: do not show checkpoint overlay (lock happens at Q20).
  ...
  }, [inSet, done, blockAnswered, blockCorrect, checkpointOpen, isFreeUser]);
  ...
  // Locked (or normal no access) => show Special Access screen
  if ((!hasAccess && !ownerOverride && freeLocked) || (!hasAccess && !ownerOverride && !isFreeUser)) {
    return (
      <div style={styles.page}>
        <div style={styles.container}>
          <div style={styles.header}>
